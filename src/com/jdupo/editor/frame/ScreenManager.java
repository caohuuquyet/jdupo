 ...
 * jDUPO
 */
package com.jdupo.editor.frame;
...
import com.jdupo.data.DataFederation;
import com.jdupo.editor.action.impl.*;
import com.jdupo.editor.dialog.*;
import com.jdupo.editor.panel.AccountabilityMessagePanel;
import com.jdupo.editor.panel.EnforcementMessagePanel;
import com.jdupo.editor.panel.GenericMessagePanel;
import com.jdupo.listener.TheoryFrameListener;
import com.jdupo.sys.EditorConst;
import com.jdupo.sys.PrintThread;
...
public class ScreenManager implements TheoryFrameListener {
	...
	private CTabFolder requestPanel = null;
	...
	public ScreenManager(final Shell shell, final CTabFolder desktop, final CTabFolder messagePanel) {
		if (null == desktop)
			throw new IllegalArgumentException("desktop is null");
		if (null == messagePanel)
			throw new IllegalArgumentException("message panel is null");
		...
		logPanel = new AccountabilityMessagePanel(messagePanel);
		consolePanel = new EnforcementMessagePanel(messagePanel);
	...
	public void newTheory() {
		...
		do {
			theoryId = EditorConst.NEW_THEORY_ID_PREFIX
					+ Utilities.getRandomString(EditorConst.NEW_THEORY_RANDOM_KEY_LENGTH);
	...
	public void groundLiteralVariables() {
		...
		try {
			...
			if (theory.getLiteralVariablesInRulesCount() > 0 || theory.getLiteralBooleanFunctionCount() > 0) {
				// theory =
				// ReasoningEngineFactory.getLiteralVariablesEvaluator().evaluateLiteralVariables(theory,
				// getExplanationLogger());
	...
	public void transformRemoveSuperiority() {
		...
		try {
			...
			if (theory.getSuperiorityCount() == 0) {
				onLogEditorCommand("Transform theory",
						theoryFrame.getText() + " - remove superiority - no superiority found");
			} else {
				TheoryNormalizer theoryNormalizer = ReasoningEngineFactory.getTheoryNormalizer(theory.getTheoryType());
				theoryNormalizer.setTheory(theory);
	...
	public void transformRemoveDefeater() {
		...
		try {
			...
			if (theory.getDefeatersCount() == 0) {
				onLogEditorCommand("Transform theory",
						theoryFrame.getText() + " - remove defeaters - no superiority found");
			} else {
				TheoryNormalizer theoryNormalizer = ReasoningEngineFactory.getTheoryNormalizer(theory.getTheoryType());
				theoryNormalizer.setTheory(theory);
	...
	public void generateConclusions() {
		...
		try {
			Theory theory = theoryFrame.getTheory().clone();
			if (theory.isEmpty()) {
				onLogEditorCommand("DataAnotation conclusions", theoryFrame.getId() + " - failed - theory is empty");
			} else {
				...
				StringBuilder sb = new StringBuilder(TextUtilities.repeatStringPattern("-", 0));// Quyet.
																								// 30
				sb.append("\nConclusions\n===========");
				for (Conclusion conclusion : conclusions) {
					...
					if (conclusion.getLiteral().getName().equalsIgnoreCase("ConsumerRequest")) {
						...
						sb.append("\n").append("ddsdsquyetsds" + conclusion.getProvabilityLevel().toString());
				...
				onLogEditorCommand("DataAnotation conclusions", theoryFrame.getId() + " - success");
				EditorFrame.getApp().currentTheoryFrameModified();
			}
		} catch (Exception e) {
			onLogEditorCommand("DataAnotation conclusions", theoryFrame.getId() + " - failed");
	...
	public void generateConclusionsWithTransformations() {
		...
		try {
			...
			if (theory.isEmpty()) {
				...
				// if (theory.getLiteralVariableCount() > 0 ||
				// theory.getLiteralBooleanFunctionCount() > 0) {
				// reasoner.removeLiteralVariables();
				// onConsoleMessage(theoryFrame.getText(),
				// "theory variables removal");
				// }
				...
				// Quyet. onConsoleMessage(theoryFrame.getText(), "transform
				// theory to regular form");

				if (theory.getDefeatersCount() > 0) {
					ps.setStartDefeaterRemoval();
					reasoner.removeDefeater();
					ps.setEndTimeDefeaterRemoval();
					// Quyet. onConsoleMessage(theoryFrame.getText(), "defeaters
					// removal");
				}
				...
				case 1:
					...
					// Quyet. onConsoleMessage(theoryFrame.getText(),
					// "superiority relations removal");
					break;
				default:
				}
				...
				onConsoleMessage(theoryFrame.getText(),
						"Load Theory Time Used =" + ps.getLoadTheoryTimeUsed() + " ("+ ps.getNoOfRules() + ","+ ps.getNoOfLiterals() +")");
				
				onConsoleMessage(theoryFrame.getText(),
						"Max. Memory used=" + ps.getMaxMemoryUsed()/ 1024);
				
				onConsoleMessage(theoryFrame.getText(),
						"Trust Computation Time Used =" + ps.getTotalTimeUsed());
				...
				if (cs) {
					...
					String responsedata = df.queryData();
					endTime = System.currentTimeMillis();
					
					// onConsoleMessage(theoryFrame.getText(), sb.toString());
					onConsoleMessage(theoryFrame.getText(),
							"Data Federation Time Used=" + Converter.long2TimeString(endTime - startTime));

					//onConsoleMessage(theoryFrame.getText(), responsedata);

					onConsoleMessage(theoryFrame.getText(), "Total Time Used="
							+ Converter.long2TimeString( ps.getTotalTimeUsed() + (endTime - startTime)));
				}
	...
	public void saveTheory() {
		...
		saveTheory(SaveUsagePolicy.LABEL, theoryFrame, theoryFrame.getTheoryFilename());
	}

	public void saveTheoryAs() {
		TheoryFrame theoryFrame = (TheoryFrame) desktop.getSelection();
		saveTheory(SaveUsagePolicyAs.LABEL, theoryFrame, null);
	}
	...
	private void saveTheory(final String commandName, final TheoryFrame theoryFrame, final File filename) {
		...
		try {
			...
			if (FileManager.getFileExtension(f).equals(""))
				f = new File(f.getCanonicalPath() + ".dfl");
			if (null == f) {
	...
	private void addRule(RuleType ruleType) {
		...
		case MODE_CONFLICT:
			...
			break;
		case REQUEST:
			ruleDialog = new RuleDialog(shell, "Consumer's Request", null);
			break;
		default:
			break;
		}
		...
		try {
			...
			if (null == result)
				return;
			// System.out.println("ScreenManager.addRule.result.getClass()=" +
	...
	public void update(LiteralVariable literalVariableName) {
		...
		try {
			...
			onLogEditorCommand("Update literal variable",
					theoryFrame.getText() + ": " + literalVariableName + " - failed");
		}
	}
	...
	// if (null == explanationLogger) explanationLogger = new
	// ExplanationLogger();
